...
\begin{figure}
\center
<<data_visualisation_price, fig=TRUE, fig.hight=5cm , echo=False>>=
...
p3 <- qplot(x = clarity, y = price, data = diamonds,
      geom = 'boxplot') + coord_cartesian(ylim = c(0, 8000))
p4 <- ggcorr(diamonds_data_omited[,c('carat', 'depth', 'table', 'price', 'x', 'y', 'z')])
...
<<model_simple, echo=TRUE, eval=TRUE>>=
...
<<model_middle, echo=FALSE, eval=TRUE>>=
...
outliers_indexes_from_simple <- strtoi(labels(outliers_from_simple$p)) #indexes of outliers
...
model_simple_out <- glm(formula = price~z+x+carat, data = diamonds_data_omited_simplout,
                        family = Gamma(link = 'log'))
@
After removing outliers we did the residual analysis once more. The results are presented on the charts \ref{simple_out_model_res}. We could see that the result are better, for example the red line chart 'Residuals vs Fitted' is closer to the dashed one which meens that they are closer to the normal distribution.
...
@
\label{simple_out_model_res}
\caption{Charts presenting residual analysis for model only with continous variables but without outliers}
\end{center}
\end{figure}
