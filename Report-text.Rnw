...
\end{table}
    ...
    <<model_complex, echo=FALSE, eval=TRUE>>=
  ...
  @
    ...
    We checked if fitted models fulfill appropriate assumptions. It turns out that none of them do, but we will assume that they do. On the plots below we can see that non of them has normally distributed residuals.
    
    <<model assumptions, echo=FALSE, eval=TRUE>>=
    ...
    To find Outliers of our fits we use outlierTest function. It provides us a list of most significant outliers. We decided [???????? WHAT DO WE DO] to delete outliers that are too far [???] from rest of the data. Afret that we can try to fit the models again. (three lines below but using dataset with deleted outliers - different for each glm or fit by glmulti again, but maybe not necessarily)
    model_simple <- glm(formula = price~z+x+carat, data = diamonds_data_omited, family = Gamma(link = 'log'))
model_middle <- glm(formula = price~color+clarity+x+carat, data = diamonds_data_omited, family = gaussian())
model_complex <- glm(formula = price~carat + carat:x + carat:color + carat:clarity, data = diamonds_data_omited, family = gaussian())
  ...
  @
    ...
    To check if all parameters in our models are significant we use function summary called at specific model. Than we look at the significance codes and we can find out what parameters are significant. It looks like in the example below:
    
  <<significance_checking, echo=FALSE, eval=TRUE>>=
